...
#typecho-option-item-customCss-40,#typecho-option-item-customJs-41{
    ...
    <span style="margin-bottom:10px;display:block"> 如果您喜欢handsome主题，<a href="https://github.com/ikym/typecho-theme-handsome/stargazers" style="color: rgb(255, 255, 255); background: rgb(63, 63, 63);">给该项目star一下吧</a></span>
   ...
   $BGtype = new Typecho_Widget_Helper_Form_Element_Radio('BGtype',
    ...
    //首页名称
    $IndexName = new Typecho_Widget_Helper_Form_Element_Text('IndexName', NULL, '拾柒的部落格', _t('首页的名称'), _t('输入你的首页显示的名称'));
    $form->addInput($IndexName);
    //博主名称：aside.php中会调用
    $BlogName = new Typecho_Widget_Helper_Form_Element_Text('BlogName', NULL, 'Cyril.Wong', _t('博主的名称'), _t('输入你的名称建议为英文，中文也可'));
    ...
    //github
    $socialgithub = new Typecho_Widget_Helper_Form_Element_Text('socialgithub', NULL,'https://github.com/ikym', _t('输入github链接'), _t('在这里输入github链接,带http://，在时光机页面显示,为空则不显示按钮'));
    $form->addInput($socialgithub);

    //email
    $socialemail = new Typecho_Widget_Helper_Form_Element_Text('socialemail', NULL,'i@lia.im', _t('输入email地址'), _t('在这里输入email地址，在时光机页面显示'));
